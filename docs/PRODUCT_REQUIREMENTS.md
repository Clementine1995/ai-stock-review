...
- 已新增 `evidence collect-pool-history --date --source akshare --output-dir`：只请求真实池代码的日线，输出收盘、成交量、成交额及 5/20 日区间事实；东方财富日线失败时可降级到 AKShare 的腾讯日线，备用源的手数只写入成交量、成交额保留缺口；窗口不足时保留缺口，不扫描全市场或认定核心票。
...
- 已新增 `review draft-final`：读取同一复盘上下文并调用用户配置的 OpenAI-compatible LLM，仅选择 STEP 1-7 的本地证据字段、真实池代码和 STEP 8 预演 ID；本地固定渲染快照事实、标准字段缺口、真实池对象、STEP 职责与待确认边界，拒绝模型输出概览、市场判断、风险判断或任何自由文本结论。有效真实池非空时，模型必须逐一关联全部真实池代码，但这不表示自动重点关注。结果只打印、不写入 SQLite、不修改池子、计划或 Observation。配置从仓库根目录 `.env` 的 `LLM_BASE_URL`、`LLM_API_KEY`、`LLM_MODEL` 读取，系统环境变量同名值优先。
- 已新增 `review record-final`：仅在用户传入 `--confirmed` 时保存 STEP 10 最终应对和风险边界；必须显式关联有效真实池代码、同日 STEP 8 预演 ID、已存在计划 Markdown 的计划项标题和同日 Observation ID。关联不一致或计划项不存在时拒绝写入。
- 尚未实现任何草案自动写入，LLM 草案仍不能绕过用户确认成为正式 STEP 10 记录。

##### 2026-07-14 LLM 草案试运行结论与后续边界

- 已使用用户本地 `.env` 中的百炼 OpenAI-compatible 配置完成一次真实 `review draft-final` 调用；本项目不记录或展示 API Key。
- 已验证：当天快照事实、证据引用、全部有效真实池和 STEP 10 待确认关联可在不写 SQLite 的前提下输出；模型自由文本结论已被拒绝，本地渲染不输出买卖建议。
- 当前已知限制：模型仍可能把允许字段放到不符合 STEP 业务语义的位置；“Evidence Snapshot 标准字段缺口为无”也不等于连续时序、身份验证、晋级率或竞价等 STEP 判断证据齐备。
- 当前固定边界：本地已固定 STEP 1-7 的字段映射与硬缺口，LLM 仅整理允许字段与对象关联；同日 `YYYY-MM-DD_real_pool_stock_history.json` 的 5/20 日事实仅进入 STEP 6/7；连续 hhxg 有效交易日达到 5/5 前继续禁止多日历史判断。
- 用户后续无需补写 STEP 1-7 主观结论；优先保持每日真实采集和真实池准确。需要正式 STEP 10 时，再由用户补充 STEP 8 四类预演并显式确认。
...
- 已按当前 `stock-review.md` 的 12 项 STEP 9 模式新增本地证据契约；每项固定展示必要证据、硬缺口和失效边界。
- 当前只读取 Evidence Snapshot 已有市场、情绪、板块和个股事实；竞价、分时、前日反馈、日线形态和监管事件未接入时固定输出 `unavailable`，不确认任何买点。
- 该清单仅作为当前模板基线；STEP 9 文本发生语义变化时，必须在 M10.6 的框架变更流程中同步复核。
...
- 新建真实 Observation 时必须保存人工指定的最晚回填日期。超过该日期仍为 `pending` 时，只在查询和周报中标记为“逾期未回填”，不得自动改为 `miss`、`invalid` 或任何其他状态。
- 当用户将 Observation 回填为 `miss` 或 `invalid` 时，必须从固定原因分类中选择一项：判断错误、证据不足、观察或执行偏差、样本无效。原因分类服务周度复盘归因，不得被解释为自动交易绩效或因果结论。
- 每日复盘必须关联一个明确的 Evidence Snapshot 版本及其本地证据截点时间。截点后的新增或修正事实必须写入新快照版本，并标记为截点后事实；它们可以作为后续回填证据，但不得改写当天已经保存的 STEP 判断、预演或最终应对。
- 复盘质量报告只展示过程指标：STEP 证据覆盖率、硬证据缺口数、真实 Observation 按期回填率和有效 `hit`/`miss` 样本量。指标只提示资料是否完整、流程是否闭环，不得评分股票、推导市场结论或生成买卖建议。

验收：

- 多条真实 `hit`/`miss` 样本可正确分组并追溯到原始记录。
- 经验候选同时展示支持样本和失败/反例，不以单次命中形成规则。
- 逾期 `pending` Observation 可被识别，但不会被系统自动改写状态。
- `miss` 和 `invalid` 样本可按固定原因分类回查；周度报告可同时展示分类计数和关联 Observation ID。
- 同一交易日存在截点后新事实时，原复盘记录仍可回查到其使用的快照版本和截点时间。
- 质量报告的四项过程指标可追溯到 STEP 证据、缺口记录和真实 Observation，且不包含交易结论。
