...
- M4.5 已完成 AKShare 最小接入口：提供 `evidence collect --source akshare --scope market`，用于采集市场层 Evidence Snapshot；真实采集需先安装可选依赖。
- 当前 `stock-review.md` 实际包含 `STEP 1` 到 `STEP 10`，命令会按文件内容动态识别，不硬编码 STEP 数量。
- 当前尚未执行真实联网采集，尚未接入 Observation 或 WebUI。
...
- `tests/`：自动化测试目录，当前覆盖 STEP 识别、无 STEP 错误、日报生成、证据接入、证据缺口识别、样例导入、AKShare 标准化、SQLite 保存、池子管理和计划生成。
...
- 首选真实数据源：AKShare，用于先补市场层事实证据。
- 当前 AKShare 最小范围：上证指数、深证成指、创业板指日线；两市成交额当前来自三个指数成交额求和字段，并在快照中标记来源。
